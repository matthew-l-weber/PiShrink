...
Usage: $0 [-adhrspvzZ] imagefile.img [newimagefile.img]
  ...
  -p         Remove logs, apt archives, dhcp leases, user bash history, and ssh hostkeys
...
if [[ $prep == true ]]; then
  info "Syspreping: Removing logs, apt archives, dhcp leases, users bash history, and ssh hostkeys"
  mountdir=$(mktemp -d)
  mount "$loopback" "$mountdir"
